...
/**************************************RELATÓRIOS DE HORÁRIOS*******************************************/
    ...
    function relatorio_aluno_horario($ocorrencia){
        ...
        // Monta query
        $query = "SELECT R.matricula, TIME_FORMAT(R.data, '%H:%m:%s') AS hora, DATE_FORMAT(R.data, '%d/%m/%Y') AS data FROM Registro_Horario R INNER JOIN Ocorrencia_Horario O ON R.RHid = O.RHid AND O.OHid = $ocorrencia";
    ...
    function relatorio_turma_horario(){
        ...
        $this->pdf->Cell(40,10,iconv('utf-8','iso-8859-1',"MATRÍCULA"),1,0,"C"); //MATRICULA
        $this->pdf->Cell(50,10,"NOME",1,0,"C"); //NOME
        ...
        foreach($resposta as $row)
        {
            $this->pdf->Cell(10, 10, "", 0, 0,"C");
            $this->pdf->Cell(40,10,$row['matricula'],1,0,"C");
            $this->pdf->Cell(50,10,$row['cname'],1,0,"C"); //ORACLE
            $this->pdf->Cell(40,10,$row['data'],1,0,"C");
            $this->pdf->Cell(40,10,$row['hora'],1,1,"C");
        }
